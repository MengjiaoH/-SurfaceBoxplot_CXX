...
class Network2(nn.Module):
    def __init__(self, n_members, num_encoder_layers, num_decoder_layers, latent_dim):
        ...
        for i in range(num_encoder_layers):
            if i == 0:
                ...
                first_layer = To_Latent(1, int(pos_latent_dim // 2**(num_encoder_layers - i - 1)), w0=w0, use_bias = True, is_first=True)
                ...
                layer = To_Latent(int(pos_latent_dim // 2**(num_encoder_layers - i)), int(pos_latent_dim // 2**(num_encoder_layers - i - 1)), w0=1.0, use_bias = True, is_first=False)
                self.pos_encoder.append(layer)
                self.pos_encoder.append(Sine(1.0))
        ...
        for i in range(num_encoder_layers):
            if i == 0:
                first_layer = To_Latent(n_members - 1, int(fc_latent_dim // 2**(num_encoder_layers - i - 1)), w0=w0, use_bias = True, is_first=True)
                self.fc_encoder.append(first_layer)
                self.fc_encoder.append(Sine(w0))
            else:
                layer = To_Latent(int(fc_latent_dim // 2**(num_encoder_layers - i)), int(fc_latent_dim // 2**(num_encoder_layers - i - 1)), w0=1.0, use_bias = True, is_first=False)
                self.fc_encoder.append(layer)
                self.fc_encoder.append(Sine(1.0))
        ...
        for i in range(num_decoder_layers):
            if i == 0:    
                ...
                last_layer = To_Latent(int(latent_dim // 2**(num_decoder_layers - 2)), 1, w0=1.0, use_bias = True, is_first=False)
                self.decoder.append(last_layer)
                self.decoder.append(Sine(1.0))
            else:
                # print(i, int(latent_dim // 2**(i - 1)), int(latent_dim // 2**(i)))
                layer = To_Latent(int(latent_dim // 2**(i-1)), int(latent_dim // 2**(i)), w0=1.0, use_bias = True, is_first=False)
